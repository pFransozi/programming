#
# This implementation is based on the book Algorithms And Data Structures In Action, by La Rocca.
# From the second chapter, improving priority queues using d-ary heaps.
# This is a priority queue implementation using a MAX-HEAP, which means, 
...
class d_ary_heap():
    ...
    def __init__(self, elements: List[Any]=[], priorities: List[Any]=[], branching_factor:int=2) -> None:
        ...
        if _validate_min_branching_factor(branching_factor):
...
def _validate_min_branching_factor(branching_factor:int):
    """
    The minimum branching for a d-ary tree is a binary tree, which means branching_factor = 2.
    Branching equals 1, the tree degenerates to a list.
    Less than that, it degenerates to nothing. 
    """
    return branching_factor < 2
...
heap = d_ary_heap(["Levantar", "Escovar os dentes", "Almoçar", "Dormir"],
                  [2, 3, 4, 1], 2)

print(f"Tarefa {heap.peak()[0]} tem prioridade {heap.peak()[1]}")